...
export default function Navbar() {  
    ...
    return (
        ...
        <nav className={`p-6 rounded-b-3xl sticky top-0 hidden md:flex justify-between max-w-7xl m-auto z-10 ${scrollPosition > 0 ? 'bg-navbg' : ''}`} style={{ backgroundColor: `rgba(52, 53, 59, ${navbarOpacity})` }}>
            <div className='w-1/2'>
                <h1 className='text-white w-fit text-2xl font-bold underline underline-offset-8 decoration-sky-500 decoration-4 -rotate-2 transform transition duration-300 ease-in-out hover:-rotate-0 hover:scale-110 cursor-pointer'>Noah Dang 👨‍💻</h1>
            </div>
        ...
        <nav className={`md:hidden sm:flex bg-navbg fixed bottom-4 z-10 w-full p-6 rounded-full flex items-center justify-between`} >
            <div className='w-2/5'>
                <h1 className='text-white w-fit text-xl font-bold underline underline-offset-8 decoration-sky-500 decoration-4 -rotate-2 transform transition duration-300 ease-in-out hover:-rotate-0 hover:scale-110 cursor-pointer'>Noah Dang 👨‍💻</h1>
            </div>
